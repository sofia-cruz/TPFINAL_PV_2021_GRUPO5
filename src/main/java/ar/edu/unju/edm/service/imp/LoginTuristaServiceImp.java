...
public class LoginTuristaServiceImp  implements UserDetailsService{
	...
	public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
		...
		// da detalles de un usuario
					Turista turistaEncontrado = turistaDAO.findByEmail(email).orElseThrow(() -> new UsernameNotFoundException("Usuario inválido"));
					System.out.println("--------------------------");
					List<GrantedAuthority> tipos = new ArrayList<>();
					GrantedAuthority grantedAuthority = new SimpleGrantedAuthority(turistaEncontrado.getRol());
